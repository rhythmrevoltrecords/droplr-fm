...
export async function releaseDayEmail(args: {
  ...
  const pst = await signToken({ ps: args.preSaveId }, "60d");
  const unsub = await signToken({ ps: args.preSaveId, act: "unsub" }, "365d");
  const utm = "utm_source=release_email&utm_medium=email&utm_campaign=presave";
  const accent = args.accentColor ?? "#8B5CF6";
  const buttons = args.platforms.slice(0, 5).map((p) => {
    const href = `${args.linkBase}/api/r/${args.releaseId}/${p}?${utm}&pst=${pst}`;
...
<img src="${args.coverUrl}" width="220" height="220" alt="" style="border-radius:14px;display:block;margin:0 auto 20px;width:220px;height:220px;object-fit:cover"/>
...
<tr><td style="padding:12px 24px 28px;text-align:center"><a href="${allLink}" style="color:${accent};font:600 14px system-ui,sans-serif">All platforms</a></td></tr>
...
<a href="${unsubUrl}" style="color:#a1a1aa">Unsubscribe</a></td></tr>
</table></body></html>`;
